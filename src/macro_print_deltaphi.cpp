...
void macro_print_deltaphi()
{
        ...
        TH1D* h = new TH1D("h","",100,-TMath::Pi()*TMath::RadToDeg(),TMath::Pi()*TMath::RadToDeg());
        ...
        for (int i = 0 ; i < clas12->GetEntries() ; i++) {
                ...
                for (int j = 0 ; j < nkftracks ; j++) {
                        ...
                        h->Fill(TMath::RadToDeg() * p_electron->DeltaPhi(*p_kftrack));
                }
        ...
        h->Draw();
        h->SetTitle(";#Delta#phi(e^{-},KFTrack);");
}