...
void macro_print_ahdc_vars()
{
        ...
        c->Print("../plots/elastics_tot_w-cut_layerfix.pdf");

        h_ept_energy->Draw("col");
        h_ept_energy->SetTitle(";p_{T,e}(GeV);dedx (adc/mm)");
        gPad->SetLogy(0);

        c->Print("../plots/elastics_pt_electron_adc_w-cut_layerfix.pdf");
        
        h_ept_adc->Draw("col");
        h_ept_adc->SetTitle(";p_{T,e}(GeV);adc");
        gPad->SetLogy(0);

        c->Print("../plots/elastics_pt_electron_pathadc_w-cut_layerfix.pdf");

        h_adc_tot->Draw("col");
        h_adc_tot->SetTitle(";ToT;adc");
        gPad->SetLogy(0);

        c->Print("../plots/elastics_tot_pathadc_w-cut_layerfix.pdf");

        h_adc_let->Draw("col");
        h_adc_let->SetTitle(";let;adc");
        gPad->SetLogy(0);

        c->Print("../plots/elastics_let_pathadc_w-cut_layerfix.pdf");

        h_tot_let->Draw("col");
        h_tot_let->SetTitle(";let;tot");
        gPad->SetLogy(0);

        c->Print("../plots/elastics_let_tot_w-cut_layerfix.pdf");
}