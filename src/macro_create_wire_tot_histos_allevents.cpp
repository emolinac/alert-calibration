...
void macro_create_wire_tot_histos_allevents()
{
        gStyle->SetOptFit(1);

        TFile* f    = new TFile(input_file_name.c_str());
        TFile* fout = new TFile("../output-files/histos_wire_tot_chisquare-allwires_10600mev.root","RECREATE");
        gROOT->cd();
        
        TNtuple* ntuple_elastics = (TNtuple*) f->Get("ntuple_allevents");
        TNtuple* ntuple_fit      = new TNtuple("ntuple_fit","","sector:layer:component:totCorr:dtotCorr:chi2:chi2ndf:extra");
        ...
        for (int j = 1 ; j <= nlayers_adc ; j++) {
                for (int i = 1 ; i <= nwires ; i++) {
                        TCut wire_cut = Form("ahdc_component==%i&&ahdc_adc_layer==%i", i, layers[j - 1]);

                        ntuple_elastics->Project("h_tot", "ahdc_timeOverThreshold", wire_cut);
                        ...
                        fout->cd();
                        h_tot->Write(Form("h_tot%i_%i",i,layers[j - 1]));
                        gROOT->cd();
                        ...
                        ntuple_fit->Fill(1, layers[j - 1], i,fitf->GetParameter(3), fitf->GetParError(3), fitf->GetChisquare(), fitf->GetChisquare()/fitf->GetNDF(), 0);
