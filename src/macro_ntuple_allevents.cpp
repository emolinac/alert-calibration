...
std::string input_file_name = "../../input-files/reco_clas_10600mev.root";

void macro_ntuple_allevents(double beam_energy_run = 10.6)
{        
        TFile* fin  = new TFile(input_file_name.c_str());
        TFile* fout = new TFile("../output-files/ntuple_allevents_He4_10600mev.root","RECREATE");
        gROOT->cd();

        TNtuple* ntuple_allevents = new TNtuple("ntuple_allevents","","ahdc_trackid:ahdc_kftrackid:ahdc_component:ahdc_layer:ahdc_superlayer:ahdc_sumadc:ahdc_leadingEdgeTime:ahdc_timeOverThreshold:ahdc_kftrackdedx:ahdc_kftrackpath:ahdc_adc_layer:kftrack_pt:Q2:W:kftrack_theta");
	...
	clas12->SetBranchAddress("ahdc_adc_timeOverThreshold", ahdc_adc_timeOverThreshold);
        ...
        for (int i = 0 ; i < clas12->GetEntries() ; i++) {
                ...
                double Q2 = 4. * beam_energy_run * electron_energy_final * std::pow(std::sin(electron_theta/2.), 2);

                double x = Q2 / 2. / (mass_target * (beam_energy_run - electron_energy_final));
                ...
                for (int i = 0 ; i < nkftracks ; i++) {
                        ...
                        for (int j = 0 ; j < n_ahdcrecohits ; j++) {
                                ...
                                for (int k = 0 ; k < nahdcrows ; k++) {
                                        ...
                                        ntuple_allevents->Fill(ahdc_trackid[j], ahdc_kftrackid[i], ahdc_wire[j], ahdc_layer[j], ahdc_superlayer[j],
                                                              ahdc_adc_sumadc[k], ahdc_adc_leadingEdgeTime[k], ahdc_adc_timeOverThreshold[k], ahdc_kftrackdedx[i], 
                                                              ahdc_kftrackpath[i], ahdc_adc_layer[k], p_kftrack->Pt(), Q2, W, p_kftrack->Theta() * TMath::RadToDeg());
                                }
                        }
                }
